...
export default function AdminUsersPage() {
  ...
  return (
    <AdminPageShell eyebrow="" title="Users" description="Review patient, consultant, and admin accounts across the platform.">
      ...
      {loading ? <AdminTableSkeleton /> : users.length === 0 ? <AdminEmptyState title="No users found" message="Users will appear here when the admin users API is connected." /> : (
        <div className="overflow-hidden border border-border bg-card">
          <div className="grid grid-cols-[1.2fr_1.4fr_0.7fr] border-b border-border px-5 py-3 font-sans text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">
            <span>Name</span><span>Email</span><span>Role</span>
          </div>
          {users.map((user) => (
            <div key={user.id} className="grid grid-cols-[1.2fr_1.4fr_0.7fr] items-center border-b border-border px-5 py-4 last:border-b-0">
